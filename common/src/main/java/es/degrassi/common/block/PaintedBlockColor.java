...
public class PaintedBlockColor implements BlockColor, ItemColor {
    ...
    public int getColor(ItemStack itemStack, int tintIndex) {
        if (itemStack.getTag() != null && itemStack.getTag().contains(ConduitNBTKeys.BLOCK_ENTITY_TAG)) {
            CompoundTag blockEntityTag = itemStack.getTag().getCompound(ConduitNBTKeys.BLOCK_ENTITY_TAG);
            if (blockEntityTag.contains(ConduitNBTKeys.PAINT)) {
                Block paint = PaintUtils.getBlockFromRL(blockEntityTag.getString(ConduitNBTKeys.PAINT));
                if (paint == null) {
                    return 0;
                }
