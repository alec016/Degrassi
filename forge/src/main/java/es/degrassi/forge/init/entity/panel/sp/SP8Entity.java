...
public class SP8Entity extends SolarPanelEntity {
  private LazyOptional<IItemHandler> lazyItemHandler = LazyOptional.empty();
  private LazyOptional<AbstractEnergyStorage> lazyEnergyHandler = LazyOptional.empty();
  public SP8Entity(BlockPos pos, BlockState state) {
    super(
      EntityRegister.SP_TIER_8.get(),
      pos,
      state,
      Component.translatable("block.degrassi.solar_panel_tier_8"),
      DegrassiConfig.get().solarPanelConfig.sp8_capacity,
      DegrassiConfig.get().solarPanelConfig.sp8_transfer
    );
  ...
  public int getGeneration() {
    return DegrassiConfig.get().solarPanelConfig.sp8_generation;
  }
