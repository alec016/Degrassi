...
public class SP5Entity extends SolarPanelEntity {
  private LazyOptional<IItemHandler> lazyItemHandler = LazyOptional.empty();
  private LazyOptional<AbstractEnergyStorage> lazyEnergyHandler = LazyOptional.empty();
  public SP5Entity(BlockPos pos, BlockState state) {
    super(
      EntityRegister.SP_TIER_5.get(),
      pos,
      state,
      Component.translatable("block.degrassi.solar_panel_tier_5"),
      DegrassiConfig.get().solarPanelConfig.sp5_capacity,
      DegrassiConfig.get().solarPanelConfig.sp5_transfer
    );
  ...
  public int getGeneration() {
    return DegrassiConfig.get().solarPanelConfig.sp5_generation;
  }
