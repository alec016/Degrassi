...
public class SP1Entity extends SolarPanelEntity {
  private LazyOptional<IItemHandler> lazyItemHandler = LazyOptional.empty();
  private LazyOptional<AbstractEnergyStorage> lazyEnergyHandler = LazyOptional.empty();
  public SP1Entity(BlockPos pos, BlockState state) {
    super(
      EntityRegister.SP_TIER_1.get(),
      pos,
      state,
      Component.translatable("block.degrassi.solar_panel_tier_1"),
      DegrassiConfig.get().solarPanelConfig.sp1_capacity,
      DegrassiConfig.get().solarPanelConfig.sp1_transfer
    );
  ...
  public int getGeneration() {
    return DegrassiConfig.get().solarPanelConfig.sp1_generation;
  }
