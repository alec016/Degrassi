...
public class EnergyExtendedData implements IExtendedConduitData<EnergyExtendedData> {
    ...
    public CompoundTag serializeNBT() {
        ...
        tag.putInt(ConduitNBTKeys.ENERGY_MAX_STORED, capacity);
        tag.putInt(ConduitNBTKeys.ENERGY_STORED, stored);
        return tag;
    }
    ...
    public void deserializeNBT(CompoundTag nbt) {
        ...
        if (nbt.contains(ConduitNBTKeys.ENERGY_MAX_STORED)) {
            capacity = Math.max(nbt.getInt(ConduitNBTKeys.ENERGY_MAX_STORED), 500);
        }
        if (nbt.contains(ConduitNBTKeys.ENERGY_STORED)) {
            stored = nbt.getInt(ConduitNBTKeys.ENERGY_STORED);
        }
    }
    ...
    LazyOptional<IEnergyStorage> getSelfCap() {
        if (!selfCap.isPresent()) {
            selfCap = LazyOptional.of(() -> new ConduitEnergyStorage(this));
        }
        return selfCap;
    }
