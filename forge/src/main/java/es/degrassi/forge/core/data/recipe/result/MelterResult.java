...
public class MelterResult extends Result {
  public MelterResult(ResourceLocation id, RequirementManager manager, int time, Advancement.Builder advancement, ResourceLocation advancementId) {
    super(id, manager, time, advancement, advancementId, RecipeRegistration.MELTER_SERIALIZER.get());
  }
  ...
  public void serializeRecipeData(JsonObject json) {
    ...
    manager.getByType(RequirementRegistration.FLUID.get()).stream().map(req -> (FluidRequirement) req)
      .findFirst().ifPresent(req -> req.toJson(json));
  }
}
