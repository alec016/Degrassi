...
import es.degrassi.common.utils.ThrowableUtil;
...
public class OffsetHelper {
    ...
    public static Vector2i offsetConduit(int typeIndex, int maxTypes) {
        if (typeIndex >= maxTypes) {
            Degrassi.LOGGER.warn(ThrowableUtil.addStackTrace(new IndexOutOfBoundsException("higher index than existing types")));
            return Vector2i.ZERO;
        }

        if (typeIndex < 0) {
            Degrassi.LOGGER.warn(ThrowableUtil.addStackTrace(new IndexOutOfBoundsException("negative index")));
            return Vector2i.ZERO;
        }
        ...
        Degrassi.LOGGER.warn(ThrowableUtil.addStackTrace(new IndexOutOfBoundsException("fallback was applied")));
        return Vector2i.ZERO;
    }
