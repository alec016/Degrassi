...
import es.degrassi.common.utils.TooltipUtil;
...
public class FluidClientData extends IClientConduitData.Simple<FluidExtendedData> {
    ...
    private static final ResourceLocation MODEL = Degrassi.rl("block/extra/fluids");
    private static final ResourceLocation WIDGET_TEXTURE = Degrassi.rl("textures/gui/fluidbackground.png");
    ...
    private static class FluidWidget extends AbstractWidget {
        ...
        public void renderWidget(GuiGraphics guiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
            if (isHoveredOrFocused()) {
                MutableComponent tooltip = DegrassiLang.FLUID_CONDUIT_CHANGE_FLUID1.copy();
                tooltip.append("\n").append(DegrassiLang.FLUID_CONDUIT_CHANGE_FLUID2);
                if (currentFluid.get() != null) {
                    tooltip.append("\n").append(TooltipUtil.withArgs(DegrassiLang.FLUID_CONDUIT_CHANGE_FLUID3, currentFluid.get().getFluidType().getDescription()));
                }
